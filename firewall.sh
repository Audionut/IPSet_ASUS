...
case "$1" in
	...
	banmalware|fs)
		...
		if [ -n "$2" ] && [ "$2" != "reset" ] && [ "$1" != "fs" ]; then
			...
			if [ -n "$customlisturl" ]; then
				...
				listurl="https://raw.githubusercontent.com/Audionut/IPSet_ASUS/master/filter.list"
			fi
		fi
	...
	start)
		...
		if ! ipset -L -n Skynet-Blacklist >/dev/null 2>&1; then ipset -q create Skynet-Blacklist hash:ip --maxelem 900000 comment; fi
